...
pub fn widget(self: *HelpComponent) vxfw.Widget {
    return .{
        .userdata = self,
...
pub fn typeErasedEventHandler(ptr: *anyopaque, ctx: *vxfw.EventContext, event: vxfw.Event) anyerror!void {
    const self: *HelpComponent = @ptrCast(@alignCast(ptr));
    return handleEvent(self, ctx, event);
}

pub fn handleEvent(self: *HelpComponent, ctx: *vxfw.EventContext, event: vxfw.Event) anyerror!void {
    switch (event) {
        .key_press => |_| {},
        .mouse => |_| {},
        .mouse_enter => {
...
pub fn draw(self: *HelpComponent, ctx: vxfw.DrawContext) std.mem.Allocator.Error!vxfw.Surface {
    const name: vxfw.Text = .{ .text = "HelpComponent" };
